...
public class Code_03_BiggestSubBSTInTree {
    ...
    public static ReturnType process(Node head) {
        ...
        int includeItSelf = 0;
        if (leftSubTreesInfo.head == head.left &&
                rightSubTreesInfo.head == head.right &&
                head.value > leftSubTreesInfo.max &&
                head.value < rightSubTreesInfo.min)
            includeItSelf = leftSubTreesInfo.size + 1 + rightSubTreesInfo.size;
        ...
        return new ReturnType(maxSize,
                maxHead,
                Math.min(Math.min(leftSubTreesInfo.min, rightSubTreesInfo.min), head.value),
                Math.max(Math.max(leftSubTreesInfo.max, rightSubTreesInfo.max), head.value));
    }
